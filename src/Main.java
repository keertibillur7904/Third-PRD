...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        totalMarks = javaMarks + sqlMarks + aptitudeMarks;
        ...
        attendanceStatus = attendance >= 75 ? "SUFFICIENT" : "SHORTAGE";

        System.out.print("Enter fee paid: ");
        feePaid = sc.nextDouble();

        while (feePaid < 0 || feePaid > semesterFee) {
            System.out.println("Invalid fee. Enter an amount from 0 to "
                    + String.format("%.2f", semesterFee) + ".");
            System.out.print("Enter fee paid: ");
            feePaid = sc.nextDouble();
        }

        feeBalance = semesterFee - feePaid;

        feeStatus = feeBalance == 0 ? "CLEARED" : "PENDING";
